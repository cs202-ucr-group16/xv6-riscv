...
// Initializing total tickets to 0
int total_tickets = 0;
...
/**
 *
 * This function iterates through the process table and prints scheduling
 * statistics for each active process. The statistics include process ID (pid),
 * process name, number of tickets assigned to the process, and the number of
 * ticks the process has accumulated.
 *
 */
...
/**
 *
 * This function sets the number of tickets for the currently running process.
 * The function ensures that the provided ticket value does not exceed 10000,
 * then assigns the ticket value to the process's tickets. Additionally, it
 * calculates the stride and initializes the pass value for the Stride Scheduling
 * algorithm.
 *
 */

int sched_tickets(int ticket_val)
{
  // we make sure ticket val does not esceed 10000 as given in the question
  if (ticket_val <= 10000)
  {
    // process
    struct proc *p = myproc();
    // We assign the ticket to the process's tickets
    p->tickets = ticket_val;

    //  Calculate the stride for the Stride Scheduling algorithm
    p->stride = 10000 / ticket_val;
    p->pass = p->stride;
  }
  return 0;
}
...
/**
 *
 * This function iterates through the process table and calculates the total
 * number of tickets for all runnable processes. It considers only processes in
 * the RUNNABLE state.
 *
 */
...
// 3 cases to handle each scheduler based on makefile command
#if defined(LOTTERY)
    // printf("LLotter\n");
    ...
    // both values of minpass should work
    // int minPass = -1;
    int minPass = INT_MAX;
    ...
    if (current_proc != 0 && current_proc->state == RUNNABLE)
    {
      acquire(&current_proc->lock);
      // Set the current process in the CPU structure and Adjust the pass value
      c->proc = current_proc;
